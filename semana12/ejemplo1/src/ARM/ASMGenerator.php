...
class ASMGenerator {
    ...
    public function emitNativeTime($label) {
        $this->label($label);
        $this->comment("Native time(): prologo");
        $this->stpPre($this->r["FP"], $this->r["RA"], $this->r["SP"], -16);
        $this->mov($this->r["FP"], $this->r["SP"]);

        $this->comment("Native time(): syscall clock_gettime(CLOCK_REALTIME)");
        $this->li($this->r["A0"], 0);
        $this->ldrl($this->r["A1"], "native_time_spec");
        $this->li($this->r["SYS"], 113);
        $this->syscall();

        $this->comment("Native time(): retorno tv_sec en x0");
        $this->ldrl($this->r["A0"], "native_time_spec");
        $this->ldr($this->r["A0"], $this->r["A0"], 0);

        $this->comment("Native time(): epilogo");
        $this->ldpPost($this->r["FP"], $this->r["RA"], $this->r["SP"], 16);
        $this->ret();
    }

    public function emitNativeLen($label) {
        $this->label($label);
        $this->comment("Native len(array): prologo");
        $this->stpPre($this->r["FP"], $this->r["RA"], $this->r["SP"], -16);
        $this->mov($this->r["FP"], $this->r["SP"]);

        $this->comment("Native len(array): leer dimension 0 del header");
        $this->ldr($this->r["A0"], $this->r["A0"], 8);

        $this->comment("Native len(array): epilogo");
        $this->ldpPost($this->r["FP"], $this->r["RA"], $this->r["SP"], 16);
        $this->ret();
    }
